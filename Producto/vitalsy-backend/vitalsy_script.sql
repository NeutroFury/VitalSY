...
CREATE TABLE parametros_clinicos (
    ...
    ratio_carbohidratos DECIMAL(5,2) NOT NULL, -- IC
    factor_sensibilidad DECIMAL(5,2) NOT NULL, -- IS
    objetivo_glucemia_min INT DEFAULT 70,
    objetivo_glucemia_max INT DEFAULT 150,
    tiempo_accion_insulina INT DEFAULT 3,     -- Horas que dura la insulina activa
...
CREATE TABLE registros_glucemia (
    ...
    tipo_registro VARCHAR(20) NOT NULL, -- MANUAL, SENSOR_NFC, SENSOR_BLE
    tendencia VARCHAR(20),              -- TrendType: Stable, Rising, etc.
    dispositivo_id VARCHAR(100),        -- ID del sensor FreeStyle
    comentarios TEXT,
...
CREATE TABLE registros_nutricion (
    ...
    calorias_kcal INT,
    momento_dia VARCHAR(20), -- Desayuno, Almuerzo, Cena, Snack
...
CREATE TABLE registros_insulina (
    ...
    tipo_insulina VARCHAR(50), -- Rápida, Basal
    sitio_aplicacion VARCHAR(50), -- Abdomen, Brazo, Muslo
...
CREATE TABLE alertas_predicciones (
    id SERIAL PRIMARY KEY,
    usuario_id INT REFERENCES usuarios(id) ON DELETE CASCADE,
    tipo_alerta VARCHAR(50), -- PREDICCION_HIPO, PREDICCION_HIPER
